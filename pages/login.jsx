...
export default function Login() {
  function handleSignInGoogle(){
    signIn('google',{callbackUrl:"https://auth-app-wheat.vercel.app/"})
  }
  function handleSignInApple(){
    signIn('apple',{callbackUrl:"https://auth-app-wheat.vercel.app/"})
  }
  return (
        ...
        <div className="h-[40%] w-[50%]">
          ...
          <div className="flex gap-[10px] justify-between">
            <button className="sign-icon" onClick={handleSignInGoogle}>
              <Image src={googlepik} width={16}
                height={16} alt="Google" />
              <span>Sign in with Google</span>
            </button>
            <button className="sign-icon" onClick={handleSignInApple}>
              <Image src={applepik} width={16}
                height={16} alt="Google" />
              <span>Sign in with Apple</span>
            </button>
          </div>
          <div className='user-form'>
              <label htmlFor="email" className='w-[100%] block mb-[10px]'>Email address</label>
              <input type="text" placeholder='johndoe@gmail.com' />
              <label htmlFor="pass" className='block mb-[10px]'>Password</label>
              <input type="password" placeholder='••••••••' />
              <button className='forgot-btn'>Forgot password?</button>
              <button className='submit-btn'>Submit</button>
          </div>
          <p className='register-btn'>Don’t have an account? <button>Register here</button></p>
        </div>
      </div>
    </div>
  )
